	...
	render(){
	  return (
			<div style={{overflow: 'hidden',padding:'0 10px',height:'51px',marginBottom:'10px',position: 'relative',backgroundColor:'#fff'}}>
				<Link to={{pathname:this.state.path||'/one'}}><div style={{float:'left',paddingTop:'15px'}}><img style={{width:'12px'}} src={bb}/></div></Link>
				...
				<div style={{float:'right',paddingTop:'15px'}}><Link to={{pathname:'/five'}}><img style={{width:'24px'}} src={cc}/></Link></div>
			</div>
		);
	}
}
