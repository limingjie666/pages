...
class Six extends React.Component{
	...
	onchangs(al){
		this.setState({information_type:al,type:al})
	}
	onclick(al){
		var list=this.state.list
		list.push({relationship:al.val1,name:al.val2,money:al.val3,type:{on:this.state.index}})
	}
	submission(){
		this.state.list.map((time,index)=>{
			this.state.matter.push({
				img:'',
				xm:'王大锤搬家请客',
				mar:'待参加',
				type:{
	...
	render(){
		var olistDiiv=this.state.list.map((time,index)=>{
			return (<div key={index} style={{height:'44px',lineHeight:'44px',padding:'0 5px'}}>
				...
				<span style={{marginRight:'10px'}}>{time.name+'-'+time.relationship+'-￥'+time.money}</span>
			</div>)
		})
	  return (
		<div className="tow" style={{position:'relative'}}>
			...
			{this.state.type?<div onClick={()=>{this.setState({information_type:true})}} style={{height:'44px',backgroundColor:'rgb(215, 215, 215)',lineHeight:'44px'}}>
